...
function draw_ball(ctx, ball) {
    ...
    ctx.beginPath();
    ctx.arc(ball.posx+BALL_RADIUS, ball.posy, BALL_RADIUS*.75, -1.4 * Math.PI, -.6 * Math.PI);
    ctx.stroke();

    ctx.beginPath();
    ctx.arc(ball.posx-BALL_RADIUS, ball.posy, BALL_RADIUS*.75, 1.6 * Math.PI, .4 * Math.PI);
    ctx.stroke();
}
...
export function play_beep() {
    ...
    oscillator.type = 'triangle';  // Sine wave produces a pure, clean tone
